    ...
    el.onclick = () => {
        if (document.querySelector('[data-toggle][data-active="true"]') != el) {
            ...
            setTimeout(() => {
                ...
                document.querySelector(el.dataset.toggle).classList.add("visible");
                // Opacity animation will not display if it's triggered at the same as the display changes to "block". So we have to delay it a bit
                setTimeout(() => {
...
document.querySelectorAll("[data-toggleMenu]").forEach(el => {
    el.onclick = () => {
        document.querySelector(`[data-toggle="${el.dataset.togglemenu}"]`).onclick.apply();
    }
});
...
document.querySelectorAll("[data-project]").forEach(el => {
    el.onclick = () => {
        document.querySelector(`.project-details[data-project="${el.dataset.project}"]`).classList.add("visible", "fade");
        document.body.classList.add("no-scroll");
        document.querySelectorAll('main a, .dropdown').forEach(mainEl => {
            mainEl.setAttribute("tabindex", "-1");
        });
    }
});
document.querySelectorAll(".proj-det-exit").forEach(el => {
    el.onclick = () => {
...
document.querySelectorAll(".project-details-container").forEach(el => {
    el.onclick = () => {
...
document.querySelectorAll("header label").forEach(el => {
    el.onclick = () => {
        document.querySelector(`header input#${el.getAttribute("for")}`).checked = true;
